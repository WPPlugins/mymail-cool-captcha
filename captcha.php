...
Plugin URI: http://rxa.li/mymail?utm_campaign=wporg&utm_source=MyMail+Cool+Captcha+for+Forms
...
?>
	<table class="form-table">
		...
		<tr valign="top">
			...
			<td>
				<?php
				printf('<br><img src="%s" width="' . mymail_option( 'coolcaptcha_width' ) . '" height="' . mymail_option( 'coolcaptcha_height', 70 ) . '" style="border:1px solid #ccc">', add_query_arg(array(
...
<?php
	...
	public function form_fields( $fields, $formid, $form ) {
		...
		if ( is_user_logged_in() && mymail_option( 'coolcaptcha_loggedin' ) ) { return $fields;
		}

		if ( ! in_array( $formid, mymail_option( 'coolcaptcha_forms', array() ) ) ) { return $fields;
		}
	...
	public function get_field( $html, $formid, $form ) {
		...
		if ( empty( $form->inline ) ) { $html .= '<label for="mymail-_coolcaptcha-' . $formid . '">' . $label . '</label>';
		}
		$img = sprintf('<div><img title="' . __( 'click to reload', 'mymail_coolcaptcha' ) . '" onclick="var s=this.src;this.src=s.replace(/nocache=\d+/, \'nocache=\'+(+new Date()))" src="%s" style="cursor:pointer;width:%dpx;height:%dpx"></div>', add_query_arg(array(
			'action' => 'mymail_coolcaptcha_img',
			'nocache' => time(),
			'formid' => $formid,
